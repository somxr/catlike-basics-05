...
public static class FunctionLibrary
{
	...
	public enum FunctionName { Wave, MultiWave, Ripple, Sphere, Torus, TorusKnot }
	// public enum FunctionName { Sphere, Torus }


	static Function[] functions = { Wave, MultiWave, Ripple, Sphere, Torus, TorusKnot };
	...
	{
		// p and q determine the winding of the knot
		float p = 2f;
		float q = 3f;

		// We calculate a radius that varies with time to make it "move"
		float r1 = 0.5f + 0.1f * Sin(PI * (6f * u + 0.5f * t));
		float r2 = 0.2f;

		// The s factor wraps the plane into a tube
		float s = r1 + r2 * Cos(PI * v);

		Vector3 point;
		// The knot winding logic (p and q)
		point.x = s * Sin(p * PI * u);
		point.y = r2 * Sin(PI * v);
		point.z = s * Cos(q * PI * u);
		return point;
	}
}